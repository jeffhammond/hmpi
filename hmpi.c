...
static inline int match_probe(int source, int tag, HMPI_Request* send_req) {
    HMPI_Item* cur;
    HMPI_Request req;
...
int HMPI_Iprobe(int source, int tag, HMPI_Comm comm, int* flag, HMPI_Status* status)
{
    ...
    *flag = match_probe(source, tag, &send_req);
    if(*flag) {
        if(status != HMPI_STATUS_IGNORE) {
...
int HMPI_Isend(void* buf, int count, MPI_Datatype datatype, int dest, int tag, HMPI_Comm comm, HMPI_Request *request)
{
   ...
   //     dest_node_rank = MPI_ANY_SOURCE;
    /*} else*/ {
        int diff = dest - comm->node_root;

        if(diff >= 0 && diff < comm->node_size) {
            dest_node_rank = diff;
        } else {
            dest_node_rank = MPI_UNDEFINED;
        }
    }
#endif
